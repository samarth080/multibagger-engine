...
## Addendum 23 — Valuation weight: strongest lead yet, still below the bar

**Post-hoc hypothesis, tested on the data that generated it.** From the live top
25: NIVABUPA earns 5.6% on capital, trades at 122x, its own 3-year forecast says
−4.1%/yr, and it ranks **#22 of 250**. CARTRADE is the same shape — 5.0% ROCE at
59.5x, #24. The composite rewards revenue growth (Growth 0.26) and small size
(0.16) while under-penalising the multiple (Valuation 0.14).

Two step sizes only, to limit fishing. Pinned universes, in-process, 0.05 margin.

| variant | us-sample | us-sample2 | india-primary | india-repl | clears |
|---|---|---|---|---|---|
| baseline | −0.012 | +0.014 | +0.099 | +0.137 | — |
| valuation-mid (G .20 / V .20) | +0.008 | +0.001 | +0.013 | +0.011 | 0/4 |
| valuation-swap (G .14 / V .26) | +0.012 | +0.002 | +0.031 | +0.033 | 0/4 |

**Positive in 4/4 samples, and monotonic in dose** — the full swap beats the
half step, which beats baseline, in every sample. Random noise does not usually
produce a consistent dose-response across four independent samples, which makes
this the strongest weight signal seen so far.

It is still **0/4 against the pre-registered margin.** Largest delta +0.033.
Sign test on 4/4 gives p = 0.125.

**No weight changed.** Three reasons, and the first is sufficient: it does not
clear the bar, and acting on sub-threshold deltas is the error the protocol
exists to prevent. Second, the hypothesis was generated by looking at the
outcome, so the test is confirmatory rather than predictive. Third, India
carries most of the effect (+0.031/+0.033 vs +0.012/+0.002), and India's base IC
is itself windowing-sensitive (Addendum 13).

**Recorded as the single best candidate to test on fresh, delisting-inclusive
data.** Unlike the Momentum lead (Addendum 21: 3/4, non-monotonic, max +0.041),
this one has a coherent mechanism, a visible failure motivating it, directional
agreement in every sample, and dose-response.

---

## Addendum 24 — policy tailwinds have never worked

Raised by a direct question about the catalyst/policy feature. Three findings,
all silent failures:

1. **The per-stock report section is a stub.** `Catalysts & Policy Tailwinds` in
   `report/markdown.py` renders the literal text *"modules arrive in v0.3 — this
   section will populate automatically."* It never populates. Every report
   published to date carries that placeholder.
2. **Site policy tagging cannot match, ever.** `PIB_RSS_URL` serves Hindi
   headlines while `POLICY_KEYWORDS` matches lowercase English. In the current
   build **0 of 12** items tagged to any sector. Checked `Lang=1/2/3` and
   `Regid=1..6`: every variant returns Hindi or an empty feed, so this is not a
   parameter fix — it needs an English-language source.
3. **The failure was invisible.** The build printed `12 policy items` and the
   page rendered them under a "Government policy (PIB)" heading, implying sector
   relevance that had never been computed.

**Working, for contrast:** company news attaches to 24/25 picks, and the curated
sector themes in `scoring/sector_themes.py` render correctly with their
`CURATED_AS_OF` date visible — which is the pattern the policy feed should have
followed.

**Shipped:** `build_site.py` now reports the tagged count alongside the raw count
and prints an explicit warning when items exist but none tag, with the root
cause in a comment. Same principle as `MIN_ANALYZED` refusing to publish a
