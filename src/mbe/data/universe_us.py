...
def sample_evenly(pool: list[str], size: int) -> list[str]:
    """Deterministic even-stride sample — reproducible, no head-of-list bias."""
    if len(pool) <= size:
        return list(pool)
    stride = len(pool) / size
    return [pool[int(i * stride)] for i in range(size)]
